...
<style>table {width: 80%;}td a{display:block;color:red;}td,th {height: 20px;text-align: centre;padding: 5px;}
tr:nth-child(even) {background-color: #f2f2f2}</style>
    <script type="text/javascript">
		...
		function paymentforbooking(booking_id)
		{
				...
				<div class="menu"> 	
					<div class="top-nav"> 
							<ul>
								...
								<li><a href='index.php#findCars'>Find Cars</a></li>
			...
			<span class="text2" id= 'findCars'>MY BOOKINGS</span>
				  <div class="my bookings">
				  	<?php
						...
						$innerTxt="<table><tr><th>Booking Id</th><th>Car No</th><th>Date</th><th>Location</th><th>Status</th></tr>";
						if ($result->num_rows >= 1 )	
						{
							while($row = $result->fetch_assoc() ) 
							{
								$innerTxt=$innerTxt."<tr>";
								$innerTxt=$innerTxt. "<td>".$row["booking_id"]."</td><td>".$row["car_no"]."</td><td>".$row["booking_date"]."</td><td>".$row["booking_pickup_location"]."</td><td>".$row["booking_status"]."</td>";
								if($row["booking_status"]=="Successful")
								{
									$innerTxt=$innerTxt."<td><a onclick=deletebooking('".$row["booking_id"]."')>Cancel Booking</a></td>";
								}
								else if($row["booking_status"]=="Inprogress")
								{
									$innerTxt=$innerTxt."<td><a id='".$row["booking_id"]."' onclick=paymentforbooking('".$row["booking_id"]."')>Pay</a></td>";
								}
								else
								{
